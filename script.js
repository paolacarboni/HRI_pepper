...
//         gameState.meaningfulTurns++; // Every player attempt is considered meaningful
...
// MODIFIED: End Game Button Listener
document.getElementById('end-game-btn').addEventListener('click', () => {
    // Hide all end-game related overlays
    document.querySelector('.end-game-overlay').classList.remove('active');
    document.querySelector('.end-game-message').classList.remove('active');
    document.querySelector('.difficulty-change-overlay').classList.remove('active');
    document.querySelector('.difficulty-change-screen').classList.remove('active');
    ...
    // Transition to the pre-intro screen (Pepper image)
    screens.game.classList.remove('active');
    screens.game.classList.add('hidden'); // Ensure game screen is hidden

    screens.start.classList.remove('active');
    screens.start.classList.add('hidden'); // Ensure start screen is hidden

    screens.intro.classList.remove('active');
    screens.intro.classList.add('hidden'); // Ensure intro screen is hidden

    screens.preIntro.classList.remove('hidden'); // Make pre-intro visible
    screens.preIntro.classList.add('active');


    // Reset state for returning to start screen (existing logic)
    document.querySelectorAll('.difficulty-btn').forEach(btn => btn.classList.remove('active'));
    gameState.difficulty = null;
    elements.startBtn.disabled = true; // Require theme/difficulty again

    // --- NEW: Send 'endgame' signal to the backend via WebSocket ---
    if (websocket && websocket.readyState === WebSocket.OPEN) {
    ...
    // Update message on screen (optional, as we're going to pre-intro)
    if (websocket && websocket.readyState === WebSocket.OPEN && SELECTED_PASSION) {
        updateMessage(`Theme: ${SELECTED_PASSION}. Select difficulty to play again.`);
    } else if (websocket && websocket.readyState === WebSocket.OPEN) {
        updateMessage("Connected. Waiting for game theme...");
